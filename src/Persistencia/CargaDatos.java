...
public class CargaDatos {
    ...
    public HashMap<String, Artista> cargarArtista() throws SQLException {
        ...
        while (rs.next()) {
            ...
            PreparedStatement st2 = c.prepareStatement("SELECT * FROM album where Usuario" + rs.getString("Artista"));
            ResultSet rs2 = st2.executeQuery();
            while (rs2.next()) {
    ...
    public ArrayList<String> Seguidos(String Nickname) throws SQLException {
        ...
        st = c.prepareStatement("SELECT * FROM seguidorart where Seguidor=" + Nickname);
        rs = st.executeQuery();
        while (rs.next()) {
            claves.add(rs.getString("Seguido"));
        }
    ...
    public ArrayList<String> favtemas(String Nickname) throws SQLException {
        ArrayList<String> claves = new ArrayList<>();
        PreparedStatement st = c.prepareStatement("SELECT a.Nombre, a.Artista, t.Nombtre FROM favtema as f,tema as t,album as a where Cliente=" + Nickname + " "
    ...
    public ArrayList<String> favalbum(String Nickname) throws SQLException {
        ArrayList<String> claves = new ArrayList<>();
        PreparedStatement st = c.prepareStatement("SELECT a.Nombre, a.Artista, t.Nombtre FROM favalbum as f,album as a where f.Cliente=" + Nickname + " "
                + "and f.IdAlbum=a.Id");
        ResultSet rs = st.executeQuery();
        while (rs.next()) {
    ...
    public ArrayList<String> favlistapd(String Nickname) throws SQLException {
        ArrayList<String> claves = new ArrayList<>();
        PreparedStatement st = c.prepareStatement("SELECT pd.Nombre, pd.Usuario FROM favlistap as f, listapordefecto as pd where f.Cliente=" + Nickname + " "
                + "and f.IdLista=pd.Id");
        ResultSet rs = st.executeQuery();
        while (rs.next()) {
            claves.add(rs.getString("pd.Usuario") + ":" + rs.getString("pd.Nombre"));
        }
        rs.close();
        st.close();
        return claves;
